...
import { cargoCreateSchema, type CargoCreateForm } from '../../schemas/cargos/cargoCreate.schema';
...
export const CargoCreateModal: React.FC<CargoCreateModalProps> = ({ open, onClose, onCreated }) => {
    // HOOKS AL INICIO
    const { create, loading, error, success, clearSuccess } = useCreateCargo();
    const { register, handleSubmit, reset, setValue, formState: { errors } } = useForm<CargoCreateForm>({
    ...
    // CONDICIONAL DESPUÉS DE LOS HOOKS
    if (!open) return null;
    // handleClose después del condicional
    const handleClose = () => {
        reset();
    ...
    const handleCreate = (data: CargoCreateForm) => {
        if (selectedSeccionId === null) {
            ...
            <div className="bg-main rounded-2xl shadow-2xl p-8 w-full max-w-4xl mx-4 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-headline">Crear Cargo</h3>
                            <p className="text-sm text-paragraph">Registra un nuevo cargo en el sistema</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit(
                    ...
                    <div className="md:w-1/2 flex flex-col gap-4">
                        ...
                        {success && <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg"><FiCheckCircle className="text-green-600 flex-shrink-0" /><span className="text-green-700 text-sm">Cargo creado exitosamente</span></div>}
