...
import { userEditSchema, type UserEditForm } from '../../schemas/users/userEdit.schema';
import { UserRole, type User } from '../../../core/types/sistema-autenticacion/user.types';
import { useUpdateUser } from '../../hooks/users/useUpdateUser';
import { FiEdit3, FiX, FiEye, FiEyeOff, FiCheckCircle, FiAlertCircle } from 'react-icons/fi';
...
export const UserEditModal: React.FC<UserEditModalProps> = ({ open, onClose, user, onUpdated }) => {
    const { update, loading, error, success, setSuccess } = useUpdateUser();
    const { register, handleSubmit, formState: { errors }, reset, setValue, watch } = useForm<UserEditForm>({
    ...
    }, [success, onUpdated, onClose, reset, setSuccess]);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-main rounded-2xl shadow-2xl p-8 w-full max-w-md mx-4 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-button/10 rounded-full">
                            <FiEdit3 className="text-button text-xl" />
                        </div>
                        <div>
                            <h3 className="text-xl font-bold text-headline">Editar Usuario</h3>
                            <p className="text-sm text-paragraph">ID: {user.id}</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                {/* Form */}
                <form onSubmit={handleSubmit(data => update(user.id, data))} className="space-y-4">
                    {/* Nombre */}
                    <div>
                    ...
                    {success && (
                        <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                            ...
                            <span className="text-green-700 text-sm">Usuario actualizado exitosamente</span>
                        </div>
                    )}
