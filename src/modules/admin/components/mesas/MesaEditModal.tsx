...
import { mesaEditSchema } from '../../schemas/mesas/mesaEdit.schema';
import type { MesaEditForm } from '../../schemas/mesas/mesaEdit.schema';
import { useUpdateMesa } from '../../hooks/mesas/useUpdateMesa';
import { FiEdit2, FiX, FiCheckCircle, FiAlertCircle } from 'react-icons/fi';
...
interface MesaEditModalProps {
    ...
    mesa: Mesa | null;
    onClose: () => void;
    onUpdated?: (mesa: any) => void;
}

export const MesaEditModal: React.FC<MesaEditModalProps> = ({ open, mesa, onClose, onUpdated }) => {
    const { update, loading, error, success, clearSuccess } = useUpdateMesa();
    const { recintos } = useRecintos();
    const { register, handleSubmit, formState: { errors }, reset, watch, setValue } = useForm<MesaEditForm>({
    ...
    const [selectedRecintoId, setSelectedRecintoId] = React.useState<number | null>(mesa?.recintoId ?? null);

    // Limpiar estado al cerrar o actualizar
    const handleClose = () => {
    ...
    }, [success, onUpdated, clearSuccess]);

    // Sincronizar el select con el estado local para el mapa
    React.useEffect(() => {
    ...
    // Cuando cambie la mesa a editar, actualizar los valores del formulario y el mapa
    React.useEffect(() => {
        if (mesa) {
        reset({ numero: mesa.numero, recintoId: mesa.recintoId });
        setSelectedRecintoId(mesa.recintoId);
        }
    }, [mesa, reset]);
    ...
    return (
        ...
        <div className="bg-main rounded-2xl shadow-2xl p-8 w-full max-w-md mx-4 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-bold text-headline">Editar Mesa</h3>
            </div>
            <button
                onClick={handleClose}
            ...
            {/* Formulario */}
            <form onSubmit={handleSubmit(data => update(mesa.id, data))} className="space-y-4">
            {/* Número de mesa */}
            <div>
            ...
            {success && (
                ...
                <span className="text-green-700 text-sm">Mesa actualizada exitosamente</span>
                </div>
            )}
