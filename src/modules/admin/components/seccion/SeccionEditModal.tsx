...
import { useUpdateSeccion } from '../../hooks/seccion/useUpdateSeccion';
import { seccionUpdateSchema } from '../../schemas/secciones/seccionUpdate.schema';
import type { SeccionUpdateForm } from '../../schemas/secciones/seccionUpdate.schema';
import { GoogleMap, Polygon } from '@react-google-maps/api';
import { useGoogleMaps } from '../../../core/hooks/useGoogleMaps';
import { FiMap, FiX, FiTrash2, FiCheckCircle, FiAlertCircle, FiInfo } from 'react-icons/fi';
...
export const SeccionEditModal: React.FC<SeccionEditModalProps> = ({ 
    open, 
    onClose, 
    onUpdated, 
    seccion 
    }) => {
    const { update, loading, error, success, setSuccess } = useUpdateSeccion();
    const { register, handleSubmit, control, reset, formState: { errors } } = useForm<SeccionUpdateForm>({
        resolver: zodResolver(seccionUpdateSchema),
    ...
    React.useEffect(() => {
        ...
        reset({
            ...
            puntos: seccion.puntos?.map((punto, index) => ({
            latitud: String(punto.latitud),
            longitud: String(punto.longitud),
            orden: String(punto.orden || index + 1),
            })) || []
        });
        }
    }, [seccion, open, reset]);
    ...
    const handleUpdate = (data: SeccionUpdateForm) => {
        ...
        return update(seccion.id, dto as any);
    };
    ...
    return (
        ...
        <div className="bg-main rounded-2xl shadow-2xl p-8 w-full max-w-4xl mx-4 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                <h3 className="text-xl font-bold text-headline">Editar Sección</h3>
                <p className="text-sm text-paragraph">Modifica la información de la sección electoral</p>
                </div>
            </div>
            <button
                onClick={onClose}
            ...
            <div>
                ...
                <div>
                    <h4 className="text-lg font-semibold text-headline mb-1">Modifica la sección en el mapa</h4>
                ...
                {isLoaded ? (
                    ...
                    center={polygonPath.length > 0 ? polygonPath[0] : mapCenter}
            ...
            {success && (
                <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <span className="text-green-700 text-sm">Sección actualizada exitosamente</span>
                </div>
            )}
