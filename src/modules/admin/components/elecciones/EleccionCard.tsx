...
import { FiCalendar, FiEdit2, FiTrash2, FiFlag, FiClock, FiCheckCircle } from 'react-icons/fi';
...
export function EleccionCard({ eleccion, onClick, onEdit, onDelete }: EleccionCardProps) {
        ...
        >
            ...
            <div className="flex items-center gap-3 sm:gap-4 mb-2">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-paragraph text-xs sm:text-sm">ID: {eleccion.id}</p>
                </div>
            </div>
            ...
            <div className="space-y-2 sm:space-y-3">
                ...
                <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-secondary/30 rounded-lg">
                    <FiClock className="text-paragraph text-xs sm:text-sm flex-shrink-0" />
                ...
                <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-secondary/30 rounded-lg">
                    <FiFlag className="text-paragraph text-xs sm:text-sm flex-shrink-0" />
                    <span className={`text-xs sm:text-sm font-medium px-2 py-1 rounded-full ${getTipoColor(eleccion.tipo)}`}>
                        {eleccion.tipo}
                    </span>
                </div>
                ...
                <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-secondary/30 rounded-lg">
                    ...
                    <span className={`text-xs sm:text-sm font-medium px-2 py-1 rounded-full ${getEstadoColor(eleccion.estado)}`}>
                        {eleccion.estado}
                    </span>
                </div>
                ...
                <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-secondary/30 rounded-lg">
                    ...
                    <div className="text-paragraph text-xs">
                        <div>Creado: {formatDate(eleccion.createdAt)}</div>
                        <div>Actualizado: {formatDate(eleccion.updatedAt)}</div>
                    </div>
                </div>
            </div>
